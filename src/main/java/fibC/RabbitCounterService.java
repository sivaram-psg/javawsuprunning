 ...
 * 
 */
@WebServiceClient(name = "RabbitCounterService", targetNamespace = "http://ch03.fib", wsdlLocation = "http://127.0.0.1:8888/fib?wsdl")
public class RabbitCounterService
    extends Service
{

    private final static URL RABBITCOUNTERSERVICE_WSDL_LOCATION;
    private final static Logger logger = Logger.getLogger(fibC.RabbitCounterService.class.getName());

    static {
        URL url = null;
        try {
            URL baseUrl;
            baseUrl = fibC.RabbitCounterService.class.getResource(".");
            url = new URL(baseUrl, "http://127.0.0.1:8888/fib?wsdl");
        } catch (MalformedURLException e) {
            logger.warning("Failed to create URL for the wsdl Location: 'http://127.0.0.1:8888/fib?wsdl', retrying as a local file");
